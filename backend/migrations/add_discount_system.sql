...
CREATE TABLE IF NOT EXISTS public.discount_audit (
    ...
    approved_by UUID REFERENCES public.profiles(id),   -- manager who approved (if PIN used)
    created_at TIMESTAMP WITH TIME ZONE DEFAULT TIMEZONE('utc', NOW())
);
...
ALTER TABLE public.orders
    ...
    ADD COLUMN IF NOT EXISTS discount_approved_by UUID REFERENCES public.profiles(id);
