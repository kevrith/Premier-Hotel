...
-- =====================================================
-- Stores categories for organizing inventory items
...
CREATE TABLE IF NOT EXISTS inventory_items (
    ...
    unit_of_measure VARCHAR(50) NOT NULL, -- kg, liters, pieces, bottles, etc.
...
CREATE TABLE IF NOT EXISTS inventory_transactions (
    ...
    -- Transaction Details
    transaction_type VARCHAR(50) NOT NULL, -- purchase, sale, adjustment, waste, transfer
    ...
    reference_type VARCHAR(50), -- order, booking, manual, supplier_delivery
    reference_id UUID,
...
CREATE TABLE IF NOT EXISTS menu_inventory_mapping (
    ...
    cost_percentage DECIMAL(5, 2), -- Percentage of menu item cost
...
-- =====================================================
-- Stores alerts for low stock, expiring items, etc.
...
-- =====================================================
-- Tracks manual stock counts and audits
CREATE TABLE IF NOT EXISTS stock_audits (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),

    -- Audit Info
    audit_date DATE NOT NULL DEFAULT CURRENT_DATE,
    audit_type VARCHAR(50) DEFAULT 'manual', -- manual, scheduled, surprise
...
-- =====================================================
-- Individual items counted in an audit
...
CREATE INDEX IF NOT EXISTS idx_menu_inventory_menu ON menu_inventory_mapping(menu_item_id);
CREATE INDEX IF NOT EXISTS idx_menu_inventory_inventory ON menu_inventory_mapping(inventory_item_id);

-- Inventory Alerts
CREATE INDEX IF NOT EXISTS idx_inventory_alerts_item ON inventory_alerts(inventory_item_id);
CREATE INDEX IF NOT EXISTS idx_inventory_alerts_unresolved ON inventory_alerts(is_resolved, created_at);
...
CREATE INDEX IF NOT EXISTS idx_stock_audit_items_inventory ON stock_audit_items(inventory_item_id);
...
CREATE POLICY "Allow service role all access on inventory_categories"
    ON inventory_categories FOR ALL
    USING (true)
    WITH CHECK (true);

CREATE POLICY "Allow service role all access on inventory_items"
    ON inventory_items FOR ALL
    USING (true)
    WITH CHECK (true);

CREATE POLICY "Allow service role all access on inventory_transactions"
    ON inventory_transactions FOR ALL
    USING (true)
    WITH CHECK (true);

CREATE POLICY "Allow service role all access on menu_inventory_mapping"
    ON menu_inventory_mapping FOR ALL
    USING (true)
    WITH CHECK (true);

CREATE POLICY "Allow service role all access on inventory_alerts"
    ON inventory_alerts FOR ALL
    USING (true)
    WITH CHECK (true);

CREATE POLICY "Allow service role all access on stock_audits"
    ON stock_audits FOR ALL
    USING (true)
    WITH CHECK (true);

CREATE POLICY "Allow service role all access on stock_audit_items"
