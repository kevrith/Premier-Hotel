...
async def owner_overview(
    ...
    # Unassigned orders (no branch_id) — shown as a separate row so totals reconcile
    ...
    if unassigned_orders or unassigned_bookings:
        branch_stats.append({
            "id": None,
            "name": "Unassigned",
            "location": "Orders with no branch",
            "status": "active",
            "stats": {
                "total_revenue":    round(unassigned_fb + unassigned_room, 2),
                "fb_revenue":       round(unassigned_fb, 2),
                "room_revenue":     round(unassigned_room, 2),
                "total_orders":     len(unassigned_orders),
                "completed_orders": len(unassigned_orders),
                "total_bookings":   len(unassigned_bookings),
                "unique_customers": len({o.get("customer_id") for o in unassigned_orders if o.get("customer_id")}),
                "occupancy_rate":   0,
                "total_rooms":      0,
                "occupied_rooms":   0,
                "total_staff":      0,
