...
async def backfill_historical_sales(
    from_date: Optional[str] = None,  # YYYY-MM-DD, e.g. "2026-04-01"
    current_user: dict = Depends(get_current_user),
    supabase: Client = Depends(get_supabase_admin),
):
    """
    Backfill stock deductions for historical orders not yet logged.
    Safe to run multiple times — skips already-processed orders.
    Pass from_date=YYYY-MM-DD to limit to orders on/after that date.
    """
    user_role = current_user.get("role", "")
    if user_role not in ["admin", "manager"]:
        raise HTTPException(status_code=403, detail="Only managers and admins can run backfill")

    # All already-logged order reasons (all variants) to avoid double-deduction
    ...
    orders_query = supabase.table("orders").select(
        "id, order_number, items, status, created_at"
    ).in_("status", ["served", "completed", "delivered", "paid"])
    if from_date:
        orders_query = orders_query.gte("created_at", f"{from_date}T00:00:00")
    orders = orders_query.order("created_at").execute().data or []

    # Fetch all menu items
    items_res = supabase.table("menu_items").select(
        "id, name, stock_quantity, track_inventory"
    ).execute()
    stock_map = {i["id"]: float(i.get("stock_quantity") or 0) for i in (items_res.data or [])}
    item_meta = {i["id"]: i for i in (items_res.data or [])}

    processed = 0
    skipped = 0
    orders_to_process = []  # [(order_number, [(mid, qty, item_name)])]

    for order in orders:
        order_number = order.get("order_number") or order["id"]
        # Skip if already logged under any reason variant
        if any(f"Order {order_number} {v}" in already_logged for v in ("served", "completed", "created")):
            skipped += 1
            continue
        ...
        for oi in order_items:
            ...
            if not mid or mid not in item_meta:
                continue
            if not item_meta[mid].get("track_inventory"):
                continue
            qty = float(oi.get("quantity", 1))
            line_items.append((mid, qty, item_meta[mid].get("name", oi.get("name", ""))))

        if line_items:
            orders_to_process.append((order_number, line_items))
            processed += 1
        else:
            skipped += 1

    # Total deductions per item
    total_deductions: dict = {}
    for _, line_items in orders_to_process:
        for mid, qty, _ in line_items:
            total_deductions[mid] = total_deductions.get(mid, 0) + qty

    # running starts at current stock + total to deduct (pre-backfill level)
    running: dict = {}
    for mid, total_ded in total_deductions.items():
        running[mid] = stock_map.get(mid, 0) + total_ded

    # Build logs replaying chronologically
    final_logs = []
    for order_number, line_items in orders_to_process:
        reason = f"Order {order_number} created"
        for mid, qty, item_name in line_items:
            ...
            final_logs.append({
                ...
                "quantity_before": round(before, 3),
                "quantity_after": round(after, 3),
                "reason": reason,
                "adjusted_by": current_user["id"],
            })

    # Write final stock quantities
    for mid in total_deductions:
        new_qty = round(running.get(mid, 0), 3)
        supabase.table("menu_items").update({
            "stock_quantity": new_qty,
    ...
    for i in range(0, len(final_logs), 100):
        try:
            supabase.table("stock_adjustments").insert(final_logs[i:i + 100]).execute()
    ...
    return {
        ...
        "items_deducted": len(total_deductions),
        "log_entries_created": len(final_logs),
        "message": f"Backfill complete. {processed} orders processed, {len(total_deductions)} items deducted.",
    }
