...
export function ReceiptDisplay({ receiptData, showActions = true, guestEmail }: ReceiptDisplayProps) {
  ...
  useEffect(() => {
    api.get('/settings/tax-config')
      .then(res => setVatEnabled(res.data?.vat_enabled !== false))
  ...
  const handlePrint = () => {
    ...
    const html = `<!DOCTYPE html>
      ...
      <body>
        ...
        ${vatEnabled && receiptData.tax && receiptData.tax > 0 ? `<div class="row"><span>Tax (16% VAT)</span><span>${fmt(receiptData.tax)}</span></div>` : ''}
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <Card className="print:shadow-none print:border-0">
        <CardHeader className="text-center border-b pb-3">
          <div className="mb-2">
            <h1 className="text-2xl font-bold text-primary">Premier Hotel</h1>
        ...
        <CardContent className="pt-4 space-y-4">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              <h3 className="font-semibold text-sm mb-1.5">Guest Information</h3>
            ...
            <div>
              <h3 className="font-semibold text-sm mb-1.5">Receipt Details</h3>
              <div className="space-y-0.5 text-sm">
          ...
          {/* Itemized Charges */}
          <div>
            <h3 className="font-semibold text-sm mb-2">Itemized Charges</h3>
            <div className="border rounded-lg overflow-hidden">
              <table className="w-full">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="text-left px-3 py-2 text-xs font-medium">Description</th>
                    <th className="text-left px-3 py-2 text-xs font-medium hidden sm:table-cell">Date</th>
                    <th className="text-right px-3 py-2 text-xs font-medium hidden md:table-cell">Qty</th>
                    <th className="text-right px-3 py-2 text-xs font-medium hidden md:table-cell">Unit Price</th>
                ...
                <tbody className="divide-y">
                  {receiptData.items.map((item, index) => (
                    <tr key={index}>
                      <td className="px-3 py-1.5 text-sm">{item.description}</td>
                      <td className="px-3 py-1.5 text-sm text-muted-foreground hidden sm:table-cell">
                        {item.date ? new Date(item.date).toLocaleDateString() : '-'}
                      </td>
                      <td className="px-3 py-1.5 text-sm text-right hidden md:table-cell">
                        {item.quantity || '-'}
                      </td>
                      <td className="px-3 py-1.5 text-sm text-right hidden md:table-cell">
          ...
          <div className="space-y-1.5">
            ...
            {vatEnabled && receiptData.tax && receiptData.tax > 0 && (
              <div className="flex justify-between text-sm">
          ...
          <div className="space-y-2 text-xs text-muted-foreground border-t pt-3">
            ...
            <div className="text-center pt-2 border-t">
              <p className="font-medium text-foreground text-sm">Thank you for staying with us!</p>
              <p className="mt-0.5">accounts@premierhotel.com | +254 700 000000</p>
            </div>
          </div>
        </CardContent>
      </Card>
